/*
 * Lexi Translator — Offline AI Translation Platform
 * Copyright (C) 2024-2026 ANRMS PRIVATE LIMITED
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program. If not, see <https://www.gnu.org/licenses/>.
 *
 * COPYLEFT: Using any part of this code requires you to publish your
 * ENTIRE source code under AGPL-3.0. No exceptions. No closed-source use.
 */

package com.falconlabs.aitranslator.analytics

/**
 * No-op implementation of [LexiAnalytics].
 * Used as the default when analytics is disabled (opt-in model).
 * All method calls are intentionally empty — no data is collected or stored.
 */
class NoOpAnalytics : LexiAnalytics {

    override fun logScreenView(screenName: String) {
        // No-op: analytics disabled
    }

    override fun logEvent(name: String, params: Map<String, Any>) {
        // No-op: analytics disabled
    }

    override fun logError(error: Throwable, context: String) {
        // No-op: analytics disabled
    }

    override fun setUserProperty(key: String, value: String) {
        // No-op: analytics disabled
    }

    override fun setAnalyticsEnabled(enabled: Boolean) {
        // No-op: this implementation is always disabled
    }
}
